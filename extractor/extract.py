...
def run_pipeline():
    ...
    try:
        ...
        for codigo, nome in SERIES.items():
            ...
            logger.info(
                f"Processando {len(raw_data)} registros da série {codigo} - {nome}..."
            )
            ...
            for item in raw_data:
                try:
                    ...
                    logger.warning(
                        f"Ignorando registro invalido na serie {codigo}: {item}. "
                        f"Erro: {e}"
                    )
        ...
        logger.info(
            "Extração finalizada com sucesso. Total de %d registros "
            "inseridos na tabela raw_indicadores.",
            total_registros,
        )
